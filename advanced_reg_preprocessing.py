...
def hyperparameter_tuning(data, result):
    ...
    tuned_parameters = [{'max_depth': [5, 15, 25, 30],
                         'learning_rate':[0.001, 0.01, 0.1], 
                         'n_estimators': [100, 200, 300]}]
    search = GridSearchCV(result['xgbreg'], tuned_parameters, 
    ...
    tuned_parameters = [{'learning_rate': [0.1,1,2,3,4,5], 
                         'n_estimators': [100,200,300, 400, 500]}]   
    search = GridSearchCV(result['adbreg'], tuned_parameters, 
                          scoring='neg_mean_squared_error', n_jobs=-1, cv=3)
    search.fit(X_train, y_train)
    y_pred = search.predict(X_test)
    ...
    search = GridSearchCV(result['decreg'], tuned_parameters, 
                          scoring='neg_mean_squared_error', n_jobs=-1, cv=3)
    search.fit(X_train, y_train)
    y_pred = search.predict(X_test)
    ...
    search = GridSearchCV(result['forestreg'], tuned_parameters, 
                          scoring='neg_mean_squared_error', n_jobs=-1, cv=3)
    search.fit(X_train, y_train)
    y_pred = search.predict(X_test)
    ...
    search = GridSearchCV(result['knnreg'], tuned_parameters, 
                          scoring='neg_mean_squared_error', n_jobs=-1, cv=3)
    search.fit(X_train, y_train)
    y_pred = search.predict(X_test)
    ...
    # SVM
    print("SVM tuning..")
    tuned_parameters = [{'C':[1, 2, 3, 5, 6], 
                         'gamma': [0.0001, 0.001, 0.01, 0.1]}]  
    search = GridSearchCV(result['svmreg'], tuned_parameters, 
                          scoring='neg_mean_squared_error', n_jobs=-1, cv=3)
    search.fit(X_train, y_train)
    y_pred = search.predict(X_test)
...
def feature_selection(data, result):
    ...
    #defining grid
    grid = dict()
    grid['sel__k'] = [i for i in range(X.shape[1]-num_discarded_features, X.shape[1]+1)]
    ...
    for key in result.keys():
        ...
        if key != "performance":
            ...
            search = GridSearchCV(pipeline, grid, scoring='neg_mean_squared_error', n_jobs=-1, cv=3)
